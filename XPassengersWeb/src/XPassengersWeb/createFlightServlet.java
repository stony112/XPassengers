...
public class createFlightServlet extends HttpServlet  {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2565051276021289034L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ResultSet airplanes = dao.getSingleContent("*", "airplanes", pID);
			ResultSet pilot = dao.getSingleContent("*", "pilots", activePilot);
			int pilotLicense = pilot.getInt("licenseID");
			int planeLicense = airplanes.getInt("minLicense");
			if (pilotLicense >= planeLicense) {
				String airplane = airplanes.getString("name");
				...
				param.put("created", "true");			
			} else {
				param.put("created", "false");
			}			
			utils.redirect(request, response, param);
